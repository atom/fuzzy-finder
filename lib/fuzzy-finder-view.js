  ...
  constructor () {
    ...
    this.selectListView = new SelectListView({
      ...
      elementForItem: ({filePath, projectRelativePath}) => {
        ...
        let classList = []
        const addIconToElement = IconServices.get('element-icons')
        if (addIconToElement) {
          if (!this.iconDisposables) {
            this.iconDisposables = new CompositeDisposable()
            this.subscriptions.add(this.iconDisposables)
          }
          this.iconDisposables.add(addIconToElement(primaryLine, filePath))
        } else {
          const iconClasses = IconServices.get('file-icons').iconClassForPath(filePath, 'fuzzy-finder')
