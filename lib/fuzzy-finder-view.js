...
const NativeFuzzy = require('nuclide-prebuilt-libs/fuzzy-native')
...
module.exports = class FuzzyFinderView {
  constructor () {
    ...
    this.nativeFuzzy = new NativeFuzzy.Matcher(this.items.map(el => el.uri))
        ...
        if (this.useAlternateScoring) {
          this.selectListView.update({
            filter: (items, query) => {
              ...
              return (
                this.nativeFuzzy.match(query, {maxResults: MAX_RESULTS}).map(
                  el => this.convertPathToSelectViewObject(el.value)
                )
              )
  ...
  setItems (items) {
    // This potential optimization is useless, since every time this function
    // is called we receive a new instance of the array :(
    if (items === this.items) {
      return
    }
    ...
    // This operation is very slow for large projects!!
    this.nativeFuzzy.setCandidates(this.items.map(el => el.filePath))
