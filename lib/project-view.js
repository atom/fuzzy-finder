...
export default class ProjectView extends FuzzyFinderView {
  ...
  async populate () {
    ...
    if (this.reloadPaths) {
      ...
      if (this.paths) {
        await this.selectListView.update({infoMessage: 'Reindexing project\u2026'})
      } else {
        await this.selectListView.update({infoMessage: 'Indexing project\u2026', infoBadge: '0'})
        if (task) {
          let pathsFound = 0
          task.on('load-paths:paths-found', (paths) => {
            pathsFound += paths.length
            this.selectListView.update({infoMessage: 'Indexing project\u2026', infoBadge: humanize.intComma(pathsFound)})
          })
        }
      }
    }
